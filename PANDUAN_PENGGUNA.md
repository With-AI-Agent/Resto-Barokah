...
| `node alat/uji-sql.mjs` | `Uji database.` | Menjalankan **uji database nyata** (PostgreSQL di dalam Node) | baris `uji: <jumlah> LULUS · 0 GAGAL` (jumlahnya bertambah setiap ada uji baru) | ada GAGAL → jangan lanjut; agent memperbaiki + menambah uji yang gagal itu sebagai uji tetap |
