...
| Database: penyewa, cabang, pengguna, izin, pengaturan, katalog, stok, meja, pesanan, pembayaran — diuji otomatis di PostgreSQL nyata (65 berkas uji, semua LULUS) | Perangkat terdaftar & login staf (Fase 1B), kontrak UI (Fase 1C) |
...
- **Isi prompt pendek wajib:** URL versi tetap (SHA penuh), cabang sumber paket, commit sasaran, perintah baca SELURUH paket dan langsung jalankan tahapnya, berhenti bila paket/sasaran tidak cocok (jangan menebak), larangan ubah kode/merge/push main, kewajiban laporan **commit + push ke cabang sesi sendiri** dan bukti lokasi/commit/status push. Prompt pendek adalah pintu masuk, bukan pengganti paket lengkap.
...
- **PENTING — 1 pekerja = 1 sesi BARU (pelajaran insiden 2026-09-21):** tiap prompt pekerja ditempel di **sesi/chat Arena BARU** (base branch = cabang pekerja itu) — JANGAN menempel 2+ prompt di satu sesi yang sama. Insiden nyata: 3 pekerja berbagi 1 cabang; papan sempat salah status; panen tetap jalan karena berkas tiap pekerja berbeda, tetapi butuh kerja darurat integrator. Kalau telanjur berbagi cabang: JANGAN panik — kabari integrator apa adanya; integrator menempelkan pesan pelindung ke tiap pekerja (pull-dulu, tanpa force) lalu memanen satu per satu.
