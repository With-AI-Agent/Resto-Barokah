...
| Database: penyewa, cabang, pengguna, izin, pengaturan, katalog, stok, meja, pesanan, pembayaran — diuji otomatis di PostgreSQL nyata (21 berkas uji, semua LULUS) | Perangkat terdaftar & login staf (Fase 1B), kontrak UI (Fase 1C) |
...
| `python3 alat/periksa-buku-uji.py` | (bagian dari "uji semuanya") | Menjaga **Buku Uji Pemilik**: tiap baris punya langkah (maksimal 5), harapan, kolom hasil, dan tugas yang benar | `LOLOS` + jumlah baris | gagal → agent memperbaiki barisnya |
...
| 2026-09-17 | **Lembar kunci pemilik** (`docs/ops/DAFTAR_KUNCI_PEMILIK.local.md`, tidak ikut Git) + penjaga `alat/periksa-rahasia.py` | Permintaan Lee: tempat mengumpulkan hal yang harus disiapkan termasuk kunci; risiko sudah diperhitungkan Lee (data percobaan, rotasi sebelum rilis, repo privat) |
