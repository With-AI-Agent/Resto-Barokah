#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Uji aturan wajib desain versi APLIKASI (Fase 0, T0-04).

Sama dengan prototipe/uji-kontras.py, tetapi membaca token aplikasi
(src/gaya/token/tema.css) + tata letak aplikasi (src/gaya/komponen.css).
...
KOMPONEN_PATH = os.path.join(BASE, 'src', 'gaya', 'komponen.css')
...
periksa(css_gabung.count(':focus-visible') >= 4, 'Cincin fokus dipasang di banyak komponen',
        f'{css_gabung.count(":focus-visible")} tempat memakai --cincin')
periksa('prefers-reduced-motion' in css_gabung and 'transition-duration:.001ms' in css_gabung,
...
for fam, daftar in face.items():
    for f in daftar:
        ...
        jalur = os.path.normpath(os.path.join(BASE, 'src', 'gaya', 'token', f))
...
FOLDER_HURUF = os.path.join(BASE, 'src', 'gaya', 'aset', 'font')
ukuran = sum(os.path.getsize(os.path.join(FOLDER_HURUF, f))
             for f in os.listdir(FOLDER_HURUF) if f.endswith('.woff2'))
