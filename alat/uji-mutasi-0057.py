#!/usr/bin/env python3
"""
UJI MUTASI 0057 — membuktikan ketajaman pagar TRUNCATE catatan_audit
(N F-02 audit AUD-3 2026-09-24): pemotongan tabel catatan_audit dilarang.

Mutasi yang WAJIB membuat `supabase/tes/truncate_audit_ditolak.sql` MERAH:
  1. Trigger cegah_truncate dicabut (TRUNCATE dibiarkan lolos)
  2. Badan fungsi cegah_truncate_audit dirusak (tidak raise exception)

Kontrol positif: salinan utuh hijau.

Verifikasi: python3 alat/uji-mutasi-0057.py
            python3 alat/uji-mutasi-0057.py --uji-diri
"""
import os
import subprocess
import sys
...
MIGRASI = os.path.join(REPO, "supabase", "migrations", "0057_truncate_audit_ditolak.sql")
BERKAS_UJI = ["supabase/tes/truncate_audit_ditolak.sql"]
...
DAFTAR_MUTASI = [
    (
        "trigger cegah_truncate dicabut",
        "create trigger catatan_audit_cegah_truncate",
        "-- MUTASI: trigger dicabut\n-- create trigger catatan_audit_cegah_truncate",
    ),
    (
        "badan fungsi dirusak (tidak raise exception)",
        "raise exception using",
        "return null; -- MUTASI: tidak raise\n    -- raise exception using",
    ),
]


def uji_mutasi():
    print("UJI MUTASI 0057 (N F-02 audit 2026-09-24 — penolakan TRUNCATE catatan_audit)")
    ...
    try:
        ...
        print(
            ...
            "pagar penolakan TRUNCATE catatan_audit (N F-02) terbukti bekerja."
        )
        return 0
    finally:
        with open(MIGRASI, "w", encoding="utf-8") as f:
...
def uji_diri():
    print("UJI DIRI penilai mutasi 0057")
    with open(MIGRASI, "r", encoding="utf-8") as f:
        asli = f.read()
