...
# Yang dianggap pembacaan hanya kunci `pin` dari objek badan permintaan, bukan
# sembarang `.pin`/`{ pin: pin }` di jawaban. Regex sempit ini menutup mutasi
# B-F06 yang membuat output tampak seperti pembacaan input.
POLA_BACA_PIN = re.compile(
    r"\bbadan\s*(?:\[\s*['\"]pin['\"]\s*\]|\.\s*pin\b)"
    r"|\b(?:const|let|var)\s*\{[^}\n]*\bpin\b[^}\n]*\}\s*=\s*badan\b"
)
POLA_PERIKSA_PIN = re.compile(r"typeof|\.test\(|\.length|\.trim\(|Number\.|===|!==|==|!=|!\s*[A-Za-z_$]")
POLA_SINK_LOG_TERSELUBUNG = re.compile(
    r"globalThis\s*(?:\[\s*['\"]console['\"]\s*\]|\.\s*console\b)"
    r"|globalThis\s*\[\s*['\"]con['\"]\s*\+\s*['\"]sole['\"]\s*\]"
)
...
def baris_langgar_pin(kode: str) -> list[str]:
    ...
    for mentah in kode.splitlines():
        ...
        # PIN/body tidak boleh menjadi jawaban, pesan, maupun argumen mentah
        # ke `balasan(...)`; jalur RPC adalah satu-satunya pengecualian output.
        bocor_ke_jawaban = (
            re.search(r"\bbalasan\s*\(", baris) is not None
            and (any(re.search(rf"\b{re.escape(nama)}\b", baris) for nama in nama)
                 or re.search(r"\b(?:badan|body|payload|isi)\b", baris) is not None)
        )
        if bocor_ke_jawaban:
...
def periksa_isi(isi: str) -> list[tuple[str, bool, str]]:
    ...
    sink_tersembunyi = POLA_SINK_LOG_TERSELUBUNG.search(kode) is not None
    periksa(
        "TIDAK memakai console dalam bentuk apa pun (PIN tidak mungkin masuk log)",
        not console and not deno_out and not sink_tersembunyi,
...
def uji_diri() -> int:
    ...
    kasus: list[tuple[str, str, str | None]] = [
        ...
        (
            "log computed: globalThis['con' + 'sole']['log'](badan)",
            asli.replace(
                "  const penggunaId =",
