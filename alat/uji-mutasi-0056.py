...
CATATAN AUDIT-HONEST: filter `c.penyewa_id = public.penyewa_saya()` di
helper adalah LAPIS KEDUA setelah `cabang_ids_saya()` yang sudah filter
oleh `auth.uid()` di `pengguna_cabang`. Menghapus filter penyewa tidak
mengubah perilaku (pagar `cabang_ids_saya` sudah cukup), sehingga tidak
menjadi bukti yang berguna — uji K-1 sengaja **tidak** menyertakan
mutasi "filter penyewa dihapus" karena akan tumpul (lihat keputusan di
AUDIT_RIWAYAT.md §1b L/M/N F-04).

Mutasi yang WAJIB membuat `supabase/tes/cabang_aktif_saya.sql` MERAH:

  1. Body diganti `select null::uuid` — pagar lenyap (selalu NULL → uji
     admin_cabang/kasir/pelayan/dapur akan merah).
  2. Order by dibalik (desc) — cabang akhir jadi "aktif" (pelayan
     merangkap akan mendapat cabang kedua, bukan pertama).
  3. Limit 1 dihapus — baris pertama tidak lagi dijamin deterministik.
