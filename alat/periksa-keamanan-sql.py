...
def uji_diri() -> int:
    mutasi = [
        ('path dilepas dari definisi terakhir',
         'alter function public.penyewa_saya() reset search_path;', 'T130-path'),
        ('path berisi schema tak dipercaya',
         "alter function public.penyewa_saya() set search_path to '$user',public,pg_temp;", 'T130-path'),
        ('EXECUTE PUBLIC dibuka ulang',
         'grant execute on function public.penyewa_saya() to public;', 'T130-public'),
        ('fungsi baru mewarisi ACL default PUBLIC',
         "create function public.uji_acl_bocor() returns int language sql security definer set search_path=public,pg_temp as $$ select 1 $$;", 'T130-public'),
        ('komentar REVOKE bukan pencabutan hak',
         'grant execute on function public.penyewa_saya() to public;\n-- revoke all on function public.penyewa_saya() from public;', 'T130-public'),
        ('trigger menjadi RPC authenticated',
         'grant execute on function public.picu_pembatalan_sah() to authenticated;', 'T130-trigger'),
        ('trigger menjadi RPC anon',
         'grant execute on function public.picu_pembayaran_jujur() to anon;', 'T130-trigger'),
        ('RLS tabel dilepas',
         'alter table public.pesanan disable row level security;', ''),
        ('policy terakhir tabel dihapus',
         'drop policy penyewa_pilih on public.penyewa;', ''),
    ]
    gagal = 0
    with tempfile.TemporaryDirectory(prefix='keamanan-sql-') as tmp:
        ...
        kasus += [(n, t, MERAH_PAGAR, token) for n, t, token in mutasi]
