...
function kompilasi(teks) {
  return esbuild.transformSync(teks, {
    loader: 'ts',
    format: 'cjs',
    target: 'es2022',
  }).code
...
async function jalankan({ method = 'POST', isi, tanpaToken = false, fetchTiruan, origin, kode = code } = {}) {
  const panggilan = []
  const penangan = []
  const Deno = {
  ...
  let melempar = null
  try {
    vm.runInContext(kode, konteks, { filename: BERKAS })
  } catch (e) {
    melempar = e
  }
  ...
  return { jawab, badan, melempar, panggilan, logs }
}
...
// E18 — kalibrasi sink log: mutan computed-console HARUS terlihat oleh spy VM.
// Tanpa kasus ini, suite hanya mengamati jawaban dan log-mutant dapat lolos.
{
  const logMutan = kompilasi(
    sumber.replace(
      '  const aksi =',
  ...
  const logText = r.logs.map((args) => JSON.stringify(args)).join('')
  catat(
    'E18 mutan log body → spy mendeteksi PIN (kalibrasi B-F06)',
    logText.includes(PIN_UJI),
    `jumlahLog=${r.logs.length} loggedPIN=${logText.includes(PIN_UJI)}`,
  )
}
