...
|---|---|---|---|---|---|---|---|
| T-019 | 2026-09-18 | **Paket audit & paket review PR yang lama belum disegarkan** — `alat/periksa-paket.py` (penjaga F-11/F-12) mengecualikan paket yang terbit sebelum perbaikan 2026-09-18, karena isinya masih memuat jalur berkas yang belum ada | Aturan pengecualiannya **tertulis di kode** (`LEGACY_TANPA_ATURAN_ARTEFAK`) dan tercetak sebagai catatan setiap kali pemeriksa dijalankan; paket baru yang dibuat sesudah 2026-09-18 tidak punya pengecualian sama sekali | `python3 alat/periksa-paket.py` → **LOLOS dengan 24 paket terlacak** (18 catatan pengecualian). Catatan 2026-09-18 (diperbarui setelah lebur dua sesi `01a0b4c3` + `01a0a8a2`): angka jarak paket **tidak lagi ditulis tangan di sini** — dibaca otomatis dari baris **`Paket peninjau terbaru`** di `docs/ops/SIAP-LANJUT.md` yang ditulis `python3 alat/lanjut-sesi.py --siapkan` dan dijaga pemeriksa. Fakta terakhir: paket audit `AUD-3-2026-09-18` + review `PKT-2026-09-18-pr-01-putaran15` terbit di `52e22fc`; sejak itu yang berubah di `aplikasi/ supabase/ prototipe/` **hanya `aplikasi/alat/periksa-semua.sh`** (alat, bukan kode aplikasi) → temuan peninjau untuk kode & skema tetap berlaku, sedangkan peninjau yang mengaudit **gerbang CI** harus minta paket di tip. Perintah: `python3 alat/periksa-paket.py` · `python3 alat/lanjut-sesi.py | head -3` · `git diff --name-only 52e22fc..HEAD -- aplikasi/ supabase/ prototipe/` | **sebelum audit menyeluruh berikutnya** (paket lama boleh tetap tersimpan sebagai arsip) | agent | [ ] terbuka |
| T-018 | 2026-09-17 | **Akun & proyek Supabase + akun Cloudflare belum dibuat** (tugas ROADMAP `T0-00`) — hanya pemilik yang bisa membuat (butuh email & kata sandi) | Tidak menghambat kode: database diuji di PostgreSQL nyata lokal (41 berkas uji LULUS); yang menunggu hanya T0-08 (`periksa-header-perangkat` di Supabase nyata) & T0-09 (uji asap penyebaran) | Region diputuskan **Singapore**; nilai `anon` belum ada | **sebelum T0-08/T0-09 dijalankan** (parkir selama Lee belum siap) | pemilik (Lee) — dipandu `docs/ops/SIAP_AKUN_PEMILIK.md` | [ ] terbuka |
...
| T-020 | 2026-09-18 | **Urutan pembuatan vs pengerasan tabel `catatan_audit`** — T1-27 (migrasi rencana 0018) mengeras `catatan_audit` (hanya-tambah + rantai hash), padahal tabelnya sendiri baru **dibuat** di T1-13 (migrasi rencana 0023) → kalau urutan tugas diikuti, pengerasan mendahului pembuatan | Tidak menghambat T1-24…T1-26 (perangkat terdaftar, sesi, percobaan masuk) yang tidak menyentuh tabel audit; butir ini baru dibutuhkan tepat sebelum T1-27 dimulai | Nilai sementara: **T1-27 membuat tabelnya langsung dalam bentuk final** (hanya-tambah + rantai hash), T1-13 tinggal menambah yang kurang; alternatif: tukar urutan keduanya | **sebelum T1-27 dimulai** | agent (usulan) → pemilik (konfirmasi) | [ ] terbuka |
