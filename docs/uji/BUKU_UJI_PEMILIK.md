...
| U-04 | Menjalankan **sesi review PR independen** (satu-satunya cara Lee menilai kode tanpa membaca diff) | 1) Buka chat BARU (idealnya model berbeda) 2) Bilang `Siapkan review PR.` — agent menyebutkan nama berkas **paling baru** (`docs/uji/review-pr/PKT-…-SIAP-TEMPEL.md`); salin SELURUH isi berkas itu 3) Kirim pesan: `Laporan review sudah masuk, periksa.` | Chat peninjau menghasilkan laporan berisi temuan berlevel + skor kalibrasi; lalu agent memvalidasi laporan itu dengan `alat/review-pr.py --periksa-laporan` |  |  |
