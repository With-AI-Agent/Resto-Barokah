...
| P-02 | Menjawab butir tunggu yang memang butuh keputusan Lee (printer, daftar perangkat, tablet Android, pelatihan & admin cabang, kebijakan privasi) | 1) Buka `docs/TERTANGGUH.md` 2) Baca butir yang masih terbuka 3) Jawab di chat: `setuju semua`, atau pilih per butir | Butir tunggu berkurang tanpa menghentikan pekerjaan; jawaban tercatat di berkas itu |  |  |
...
| U-02 | Menjalankan **seluruh pemeriksaan otomatis** sendiri | 1) Buka terminal/agent 2) Jalankan `bash aplikasi/alat/periksa-semua.sh` 3) Tunggu sampai selesai | Baris terakhir berbunyi `SEMUA PEMERIKSAAN LOLOS.` — kalau ada yang GAGAL, keluarannya menyebut yang mana |  |  |
| U-03 | Menjalankan **uji database** sendiri | 1) Jalankan `node alat/uji-sql.mjs` 2) Baca dua baris terakhir | Tertulis `uji: <jumlah> LULUS · 0 GAGAL` dan `HASIL: LOLOS` |  |  |
| U-04 | Menjalankan **sesi review PR independen** (satu-satunya cara Lee menilai kode tanpa membaca diff) | 1) Buka chat BARU (idealnya model berbeda) 2) Salin SELURUH isi `docs/uji/review-pr/PKT-2026-09-17-pr-01-putaran7-SIAP-TEMPEL.md` 3) Kirim pesan: `Laporan review sudah masuk, periksa.` | Chat peninjau menghasilkan laporan berisi temuan berlevel + skor kalibrasi; lalu agent memvalidasi laporan itu dengan `alat/review-pr.py --periksa-laporan` |  |  |
