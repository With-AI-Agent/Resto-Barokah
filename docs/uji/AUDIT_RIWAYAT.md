...
|---|---|---|---|---|
| H F-01 | K-2 | Kunci jawaban kalibrasi cacat tanaman masih bisa dibaca dari dalam repo — penutupan D F-05 belum tuntas | **TERBUKA (diperbaiki sebagian 2026-09-20)** | bantah-balik: NYATA, dan lebih parah dari dugaan — salinan kalibrasi jalur mesin dulu dibuat `git worktree` sehingga `git diff` di salinan **langsung memperlihatkan baris cacat tanam**, sementara katalog `alat/kalibrasi-cacat.json` = daftar jawaban. Yang sudah diperbaiki: salinan jalur mesin dibuat lewat `git archive` + satu commit bersih + katalog dikeluarkan (`pastikan_salinan_bersih()` di `alat/audit-independen.py`), dan jalur review PR kini **gagal-tertutup** bila katalog masih di dalam repo (`alat/review-pr.py` hanya membaca dari `KALIBRASI_DIR`). Dijaga `alat/periksa-kunci-kalibrasi.py` aturan F & G (+3 mutasi uji-diri). **Sisa (butuh keputusan Lee):** memindahkan berkas katalog ke luar repo + baris daftar pensiun → `T1-44` / menunggu jawaban Lee |
