...
| H F-05 | K-3 | Tabel §1b paket audit memuat 11 baris "berkasnya TIDAK ADA: laporkan!" yang palsu (sudah tercatat D F-03, tetap terbit lagi di paket ini) | **TERBUKA** | `T1-44` — pembuat paket wajib membedakan perintah/glob dari berkas |
| H F-06 | K-3 | `docs/KEAMANAN.md` §3 butir 5 basi: `hitung_total()` ditulis "belum mendarat" padahal sudah hidup di `0014` | **TERBUKA** | dokumen keamanan diselaraskan dengan kenyataan kode → `T1-45` |
...
| H F-08 | K-4 | Rujukan jalur `src/lib/tema.ts` di Bukti T0-03 tidak bisa di-resolve dari akar repo | **TERBUKA** | perbaiki jalur bukti (bukan cacat kode) → `T1-45` |
...
| H F-10 | K-4 | Klasifikasi "K-1 produksi" pada temuan penanda-palsu melebihi eksploitabilitas produksi nyata | **TERBUKA** | koreksi tingkat klaim di riwayat/keputusan — jangan menaikkan tingkat demi kesan → `T1-44` |
...
| I F-03 | K-3 | Pemeriksa PIN meloloskan pengembalian PIN & logging lewat bracket | **TERBUKA** | perkuat `alat/periksa-fungsi-pin.py` (pemeriksa yang tumpul = gerbang palsu) → `T1-44` |
| I F-04 | K-3 | Classifier mutasi menganggap kegagalan lingkungan sebagai bukti pagar bekerja | **TERBUKA** | perkuat `alat/uji-mutasi-0015.py` (bedakan "MERAH karena pagar" dari "MERAH karena crash") → `T1-44` |
...
| I F-09 | K-2 | README memberi perintah akar setelah menyuruh masuk ke aplikasi | **TERBUKA** | `aplikasi/README.md` → `T1-45` |
| I F-10 | K-2 | Resep audit mengarahkan detached HEAD tetapi mengharuskan cabang sesi untuk penyerahan | **TERBUKA** | `docs/uji/PROTOKOL_AUDIT_INDEPENDEN.md` + prompt audit → `T1-44` |
| I F-11 | K-2 | Panduan rahasia memberi jaminan yang tidak disediakan GitHub | **TERBUKA** | `docs/ops/LANGKAH_PEMILIK_SEKARANG.md` + alur sebar → `T1-44` |
| I F-12 | K-2 | Aturan aktif pindah sesi saling mewajibkan dan melarang merge | **TERBUKA** | `docs/AGENT_OPERATING_GUIDE.md` + `PROMPT_SESI_BARU.md` → `T1-44` |
...
| I F-20 | K-3 | Pembuat paket menyebut perintah/glob sebagai berkas hilang | **TERBUKA** | `alat/audit-independen.py` (pembuat paket) → `T1-44` |
