...
|---|---|---|---|---|---|
| 2 | `alat/kalibrasi-cacat.json` | 2026-09-20 | **Lee** (2026-09-20: "Aku ikut yang terbaik menurut kamu. Klo sebaiknya dikeluarkan, silahkan keluarkan"), diusulkan audit H F-01 | berkas itu memuat pasangan `cari`/`ganti` = **kunci jawaban kalibrasi**. Selama ia hidup di dalam repo, auditor/peninjau yang membaca repo (memang pekerjaannya) bisa mencocokkan cacat yang ditanam lalu menulis skor `Ditemukan: X dari Y` yang sempurna tanpa mengulas — gerbang anti-teater kehilangan makna | **Status aktif: sudah dipindahkan ke luar repo** (`KALIBRASI_DIR`, baku `/home/user/.kalibrasi/kalibrasi-cacat.json`); salinan lamanya tetap ada di riwayat Git sebagai provenance dan **tidak dipakai** untuk menilai ketajaman. Alat membaca katalog dari luar repo dan menolak jalan bila tidak ada (gagal-tertutup); `alat/periksa-kunci-kalibrasi.py` aturan A2/F/G menolak bila katalog muncul lagi di dalam repo. Baris ini adalah riwayat keputusan, bukan risiko aktif. |
