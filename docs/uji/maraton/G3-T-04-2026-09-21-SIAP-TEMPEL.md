# MARATON G3 — T-04 — Pembekuan pesanan setelah pembayaran

- **Tugas:** T-04 · pekerja-1 · PEMERIKSA, laporan-saja.
...
- **ID klaim wajib:** A-F01, A-F03.
- **Lingkup tulis terpublikasi:** HANYA laporan berawalan `LAPORAN_G3_T04_` di folder `docs/uji/audit/`, diikuti UUID dan hash oleh pengirim. Tidak ada nomor migrasi cadangan.
...
- `docs/ops/PAPAN_TUGAS.md` baris T-04 dan bagian Gelombang 3.
...
- `supabase/migrations/0009_pesanan.sql`
...
1. Baca T-025(a), PRD M6/Aturan Bisnis 7 dan TECH_SPEC aturan uang. Jangan membuat keputusan uang baru. Telusuri urutan trigger efektif, bukan hanya migrasi pertama.
2. Jalankan kontrol utuh `node alat/uji-sql.mjs supabase/tes/beku_setelah_bayar.sql`; simpan exit dan keluaran. Kegagalan pustaka/sintaks bukan bukti pembekuan bocor.
3. Bangun fixture pesanan/pembayaran sah dari seed proyek. Reproduksi **satu statement** data-modifying CTE: INSERT pembayaran + UPDATE pesanan/item. Uji catatan header, tipe, meja_id, pelayan_id, shift_id dan catatan item (masing-masing transaksi terpisah). Sertakan pembayaran sebagian Rp1 dan lunas bila jalurnya berbeda.
4. Setelah transaksi **commit**, baca ulang baris pembayaran dan kolom yang berubah. Harus dibuktikan uang benar-benar tersimpan; jangan menyimpulkan dari UPDATE yang kemudian rollback. Pisahkan kolom nominal dan nonnominal; jangan mengklaim total berubah tanpa bukti.
5. Kontrol pembanding: perubahan yang sama sebagai statement kedua setelah bayar harus ditolak BY-201; perubahan sebelum bayar sah; progres dapur setelah bayar tetap sah. Catat role/klaim/izin dan SQLSTATE. Jika ketertiban eksekusi CTE tidak pasti, jelaskan dan ulangi terukur—jangan menyebut deterministik tanpa bukti.
6. Ulangi kasus penentu pada PostgreSQL nyata lokal (loader pgserver proyek boleh dipakai; pgcrypto tiruan wajib diungkap). PGlite dan PG nyata diberi label terpisah. Ketidaktersediaan PG = keterbatasan, bukan klaim aman.
7. Bedakan kemampuan menjalankan SQL langsung sebagai authenticated dari jalur RPC/PostgREST yang benar-benar bisa dicapai klien. Jangan menaikkan dampak/browser exploit dari probe SQL saja.
8. A-F03: periksa apakah tes/mutasi sekarang menutup satu-statement ini. Usulkan regresi minimal + satu mutan terisolasi; kalau melakukan eksperimen patch/mutan, hanya di salinan uji dan beri label eksperimen, **bukan perbaikan proyek**. Jangan menerbitkan migrasi/patch.
9. Hasil wajib per A-F01/A-F03: TERBUKTI / TERBANTAH / BELUM-TERVERIFIKASI; bukti kontrol dan skenario cacat, dampak yang terbukti, batas klaim, usulan perbaikan untuk integrator. A-F04 deadlock bukan tugas ini; jika teramati, catat terpisah tanpa mengambil alih.
...
- [ ] Tidak ada rahasia, sumber laporan penulis lain tidak ditimpa, prefix `LAPORAN_G3_T04_` tepat.
...
python3 "$RUNNER/kirim-laporan.py" --jenis audit --penanda G3_T04 --sumber arena/01a0c2c1-resto-barokah --siapkan
```
...
```sh
python3 "$RUNNER/kirim-laporan.py" --jenis audit --penanda G3_T04 --sumber arena/01a0c2c1-resto-barokah --laporan "$LAPORAN"
```
