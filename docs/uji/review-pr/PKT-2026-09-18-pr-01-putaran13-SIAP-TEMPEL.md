...
# PAKET REVIEW PR INDEPENDEN — pr-01-putaran13 — 2026-09-18
...
- **Perubahan:** 388 berkas · +52418 / −143 baris
...
**117 commit** dalam rentang ini (seluruhnya, bukan contoh):
...
|---|---|---|
| Merah | 72 | A `.github/workflows/ci.yml`, M `_sistem/validate_system.py`, A `alat/audit-independen.py`, A `alat/periksa-bersih.py` … |
| Kuning | 119 | A `alat/bantu_uji_diri.py`, A `alat/contoh-laporan-review/bagus.md`, A `alat/contoh-laporan-review/buruk-kedalaman-kata.md`, A `alat/contoh-laporan-review/buruk-klaim-kurang.md` … |
| Hijau | 197 | A `.gitignore`, M `ACCEPTANCE_TESTS.md`, M `AGENT_SYSTEM.md`, M `PANDUAN_PENGGUNA.md` … |

## 3. Klaim yang wajib kamu bantah
1. skills/desain-antarmuka/SKILL.md (3 pelajaran + sumber + daftar periksa);
2. wajib dibaca di fase DESAIN (alat/mulai-sesi.py);
3. penjaga baru aplikasi/alat/periksa-antarmuka.py (10 uji-diri) menolak keadaan
4. alat/periksa-rujukan.py: ekstensi `ts` lebih dulu dari `tsx` -> rujukan .tsx terbaca
5. aplikasi/alat/uji-kontras.py: komentar di belakang nilai membuat token berikutnya tak
6. aplikasi/alat/periksa-komponen-env.py: menyelesaikan var() sebelum menilai lantai 44 px
7. PR-01 (K-2, uang): diskon `jenis='voucher'` bisa dicatat kasir 100% subtotal tanpa
8. PR-02 (K-3, status): pesanan bisa LAHIR `batal` (tanpa jejak pembatalan → Aturan
