# PAKET REVIEW PR INDEPENDEN — pr-01-putaran9 — 2026-09-17
...
- **PR / cabang:** `pr-01-putaran9`
- **Dasar (base):** `origin/main` → **Kepala (head):** `HEAD`
- **Commit yang direview:** `b8679ecaeed36d06d9859304c05302a313015395`
- **Perubahan:** 346 berkas · +42946 / −132 baris
...
|---|---|---|
| Merah | 58 | A `.github/workflows/ci.yml`, M `_sistem/validate_system.py`, A `alat/audit-independen.py`, A `alat/periksa-bersih.py` … |
| Kuning | 112 | A `alat/bantu_uji_diri.py`, A `alat/contoh-laporan-review/bagus.md`, A `alat/contoh-laporan-review/buruk-klaim-kurang.md`, A `alat/contoh-laporan-review/buruk-tanpa-bukti.md` … |
| Hijau | 176 | A `.gitignore`, M `ACCEPTANCE_TESTS.md`, M `PANDUAN_PENGGUNA.md`, M `PROFIL_PENGGUNA.md` … |

## 3. Klaim yang wajib kamu bantah
1. CI kiriman 896c5b4 GAGAL: enam dokumen merujuk ber-backtick ke berkas kerja
2. Rujukan diarahkan ke formulir baru docs/ops/DAFTAR_KUNCI_PEMILIK.template.md
3. Penjaga baru alat/periksa-bersih.py: menyalin hanya berkas terlacak git
4. alat/periksa-rahasia.py: formulir wajib ada (uji mutasi ke-3).
5. Terpasang di periksa-semua.sh dan CI; PANDUAN_PENGGUNA Bagian E diberi baris.
6. 
7. Uji otomatis membuktikan perilaku baru/bebas regresi pada commit ini (bukan commit sebelumnya).
8. Tidak ada gerbang keamanan/CI yang dilemahkan (ambang diturunkan, uji dimatikan, revoke/hak dicabut dihapus).
9. Perubahan pada jalur uang/keamanan/data pelanggan tidak bisa dilewati lewat pemanggilan langsung (RPC/API).
10. Dokumen yang menyatakan perilaku (fondasi, buku induk, panduan Lee) sudah ikut diperbarui — tidak ada klaim basi.
11. Setiap berkas baru benar-benar dipakai (tidak ada berkas mati / rujukan menggantung).
...
3. `git diff origin/main...HEAD` — baca diff sungguhan; cari hal yang **tidak** ada di deskripsi.
