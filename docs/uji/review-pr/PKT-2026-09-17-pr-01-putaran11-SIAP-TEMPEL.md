...
# PAKET REVIEW PR INDEPENDEN — pr-01-putaran11 — 2026-09-17
...
- **Perubahan:** 369 berkas · +47630 / −132 baris
...
**108 commit** dalam rentang ini (seluruhnya, bukan contoh):
...
| Kuning | 117 | A `alat/bantu_uji_diri.py`, A `alat/contoh-laporan-review/bagus.md`, A `alat/contoh-laporan-review/buruk-kedalaman-kata.md`, A `alat/contoh-laporan-review/buruk-klaim-kurang.md` … |
| Hijau | 186 | A `.gitignore`, M `ACCEPTANCE_TESTS.md`, M `PANDUAN_PENGGUNA.md`, M `PROFIL_PENGGUNA.md` … |

## 3. Klaim yang wajib kamu bantah
1. U-04 kini berbunyi: bilang `Siapkan review PR.`, lalu salin berkas
2. Penjaga `alat/periksa-buku-uji.py` menerima pola itu hanya bila menyebut folder
3. `bash aplikasi/alat/periksa-semua.sh` memasang `aplikasi/node_modules` dan
4. Perintah uji database dibungkus `bash alat/uji-database.sh` (memasang sendiri
5. Penjaga baru di `aplikasi/alat/periksa-struktur.py`: kedua skrip WAJIB memeriksa
6. Buku Uji U-02/U-03 + log buku disegarkan; PANDUAN Bagian E menambah baris
7. `Uji semuanya.` = kalimat biasa di chat → agent menjalankan
8. `Uji database.` = kalimat biasa → agent menjalankan `node alat/uji-sql.mjs`
