...
# PAKET REVIEW PR INDEPENDEN — pr-01-putaran14 — 2026-09-18
...
- **PR / cabang:** `pr-01-putaran14`
- **Dasar (base):** `origin/main` → **Kepala (head):** `origin/arena/01a0a8a2-resto-barokah`
- **Commit yang direview:** `cdd80b6724845663103cde132273919a4636e2d4`
- **Perubahan:** 410 berkas · +56944 / −143 baris
...
**123 commit** dalam rentang ini (seluruhnya, bukan contoh):
...
| Kuning | 120 | A `alat/bantu_uji_diri.py`, A `alat/contoh-laporan-review/bagus.md`, A `alat/contoh-laporan-review/buruk-kedalaman-kata.md`, A `alat/contoh-laporan-review/buruk-klaim-kurang.md` … |
| Hijau | 205 | A `.gitignore`, M `ACCEPTANCE_TESTS.md`, M `AGENT_SYSTEM.md`, M `PANDUAN_PENGGUNA.md` … |

## 3. Klaim yang wajib kamu bantah
1. _sistem/validate_system.py menuntut field 'Pekerjaan belum tersimpan: Tidak ada' tepat
2. T-019 (paket lama belum disegarkan) wajib ditandai ❓ dari satu tugas ROADMAP;
3. F-01 hitung_total() ada (satu-satunya penulis angka uang) → alur uang tidak buntu
4. F-02/PR-01/PR-05 subtotal selalu dari peladen; dapur hanya memindahkan status masak
5. F-03 cap bawaan diskon 100% → 50% (100% bukan cap)
6. F-04 jejak pesanan: kasir_id dipaksa, tanggal hari ini & beku, nomor dari sistem,
7. PR-03 pembatalan sah menutup pesanan (batal + waktu + alasan)
8. PR-04 stempel izin diskon wajib punya bukti PIN terpakai
