# Sumber gambar referensi

Gambar di folder ini **hanya contoh inspirasi** untuk dibahas bersama pemilik, **bukan** desain produk kita.
Desain produk Resto Barokah akan dibuat sendiri (orisinil) — tidak menyalin karya orang lain.

| Berkas | Sumber (halaman publik) | Catatan |
|---|---|---|
| 01-pos-dashboard-food.webp | https://dribbble.com/tags/pos-dashboard | Daftar pesanan + kategori menu |
| 02-kasir-cashier-dashboard.webp | https://dribbble.com/tags/cashier | Panel menu + keranjang kanan (pola terpilih) |
| 03-admin-order-coffeeshop.webp | https://dribbble.com/tags/restaurant-pos-system | Papan pesanan berkolom status |
| 04-dapurku-pos-fastfood.webp | https://dribbble.com/search/restaurant-pos | Kasir mobile + kartu promo |
| 05-kds-1.webp | https://dribbble.com/tags/kitchen-display-system | Kisi menu tablet + angka stok |
| 06-kds-2.webp | https://www.behance.net/gallery/187918001 | **Slide presentasi** (bukan desain layar) — pola kartu berwarna masih berguna |
| 07-kds-3.webp | https://www.behance.net/gallery/187918001 | **Slide presentasi** (bukan desain layar) |
| 08-pos-pelayan-mobile.webp | https://dribbble.com/search/waiter-app | Kumpulan layar HP (resolusi kecil) |
| 09-waiter-booking-app.webp | https://dribbble.com/tags/waiter-app | Contoh tema hangat (oranye) |
| 10-waiter-android.webp | https://dribbble.com/tags/waiter_app | Contoh tampilan minimal satu warna |

## Tambahan (pencarian kedua, 2026-09-16, menyusul kiriman pemilik)

| Berkas | Sumber (halaman publik) | Catatan |
|---|---|---|
| 11-alur-keranjang-checkout.webp | https://www.behance.net/gallery/191213389/Food-App-Check-out-and-Cart-Page-UI | Alur HP: detail produk → keranjang → checkout (paling rapi di pencarian kedua) |
| 12-dashboard-laporan-resto.jpg | https://multipurposethemes.com/blog/restaurant-dashboard-for-orders-sales-management | Contoh layar laporan: kartu angka + grafik + tabel |
| 13-dashboard-gelap.webp | https://dribbble.com/tags/dark-admin | Contoh panel tema gelap |

## Kiriman pemilik (folder `pemilik/`)

Pemilik mengirim **34 gambar** sendiri (via `Referensi.zip`, dibongkar agent 2026-09-16) dengan kode `P01`–`P34`.
Daftar isi, penilaian, dan arti tiap kode ada di `docs/desain/referensi/pemilik/README.md`;
papan visualnya `docs/desain/papan-referensi-pemilik.jpg`. **Sumber asli tiap gambar tidak diketahui**
(kumpulan simpanan pemilik dari Pinterest/web) — keterangan "asal/isi asli" pada README adalah judul berkas saat diterima.

**Catatan kejujuran:** dari 10 gambar pertama (pencarian agent), **6 berguna** sebagai rujukan; 2 hanya slide presentasi; 2 resolusi terlalu kecil.
Dari pencarian kedua (3 gambar), hanya **11 dan 12** yang berguna; **13 sekadar contoh tema gelap**.
Kesimpulan pola desain ada di `docs/desain/RENCANA_DESAIN_UI.md`; penilaian kiriman pemilik ada di `docs/desain/PENILAIAN_REFERENSI.md`.
