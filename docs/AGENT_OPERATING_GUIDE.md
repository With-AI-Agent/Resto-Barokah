...
**Gerbang penyerahan paket independen (Lee, 2026-09-21; AL-15):** setiap kali menyerahkan paket audit/review/pemeriksaan independen, agent WAJIB mencetak prompt pendek dari paket yang sudah commit+push dan menampilkannya **langsung di chat dalam blok siap-tempel**. Menyebut lokasi atau tautan saja = penyerahan belum lengkap. Pilih jalur paket eksplisit; Locator repo + cabang sumber + SHA paket + path melalui git/gh terautentikasi wajib ada; SHA target terpisah, baca utuh/gagal-tertutup. Tanpa meminta Lee lagi, pengirim terisolasi `alat/kirim-laporan.py` wajib otomatis commit/push/verifikasi remote, termasuk cabang/working tree bersama. Dilarang menimpa/force/rebase/merge. Bukti repo/cabang/path/commit/remote_tip/hash atau TERBLOKIR yang jujur (kontrak `docs/uji/PENGIRIMAN_LAPORAN_AMAN.md`). Sebelum mengirim, periksa draf respons di luar repo dengan `alat/siapkan-pemeriksaan.py --prompt-pendek <paket> --periksa-serah <draf-respons>` lalu kirim blok yang diperiksa. Alat tidak bisa membuktikan chat sudah terkirim; tanggung jawab akhir tetap agent. Jangan mengganti paket yang sudah berjalan di sesi pemeriksa tanpa kebutuhan/izin; pengecualian Lee "tidak perlu prompt baru" dihormati, bukan alasan melupakan aturan berikutnya.
