...
- [x] T1-13 — Migrasi catatan_audit (hanya-tambah) ⚠️
  - **Tujuan:** jejak tindakan sensitif tidak bisa diubah atau dihapus siapa pun.
  - **Ref:** TECH_SPEC §4 & §9 ART-6; PRD M3
  - **File:** `supabase/migrations/0020_catatan_audit.sql`, `supabase/migrations/0029_audit_kekal_rantai.sql`, `supabase/tes/catatan_audit.sql`, `supabase/tes/audit_rantai.sql`
...
- [ ] T1-22 — Penyisiran RLS menyeluruh + uji SQL otomatis di CI ⚠️
  ...
  - **File:** `supabase/tes/rls_semua_tabel.sql` (nama rencana `sisir_rls.sql` dikoreksi ke berkas nyata Batch-2 2026-09-21), `.github/workflows/ci.yml` (langkah uji SQL)
...
- [x] T1-24 — Migrasi 0012: perangkat terdaftar + `perangkat_sah()` + RLS staf diperketat ⚠️
  ...
  - **File:** `supabase/migrations/0018_perangkat_terdaftar.sql`, `supabase/migrations/0030_sesi_dan_persetujuan_perangkat.sql`, `supabase/tes/perangkat_registrasi.sql`, `supabase/tes/sesi_dan_perangkat.sql`
...
- [x] T1-27 — Migrasi 0015: `catatan_audit` hanya-tambah + rantai hash ⚠️
  ...
  - **File:** `supabase/migrations/0029_audit_kekal_rantai.sql`, `supabase/tes/audit_rantai.sql`, `alat/periksa-audit.py`
...
- [x] T1-29 — Uji matriks peran × aksi (otomatis, semua peran) ⚠️
  ...
  - **File:** `supabase/tes/matriks_izin_6_peran.sql`, `alat/periksa-matriks-izin.py`
  - **DoD:** matriks dibuat dari daftar izin & peran (bukan ditulis satu per satu); 6 peran × semua aksi RPC diperiksa; peran tanpa izin **ditolak walau RPC dipanggil langsung**; laporan matriks tercetak di log CI; uji lulus. · **Bukti 2026-09-22:** `supabase/tes/matriks_izin_6_peran.sql`, `alat/periksa-matriks-izin.py`.
  - **Kompleksitas:** besar (4 jam)
...
- [x] T1-36 — Kunci induk: kode pemulihan darurat + pendaftaran perangkat darurat ⚠️
  ...
  - **File:** `supabase/migrations/0028_pemulihan_perangkat.sql`, `supabase/tes/pemulihan.sql`, `docs/ops/PEMULIHAN_PERANGKAT.md`
...
- [ ] T1-45 — Penutupan temuan putaran verifikasi 2026-09-19 + AUD-3 2026-09-20 (43 temuan NYATA: AUD-3 D + review PR putaran16 + AUD-3 F; **+ 31 temuan dua laporan lanjutan 2026-09-20** — laporan H sesi `01a0bbcb` dan laporan I ronde kedua `01a0bbd2`, sudah terdaftar di daftar penutup §1d/§1e `docs/uji/AUDIT_RIWAYAT.md`) ⚠️ T-025
  - **AUD-2 terbaru (2026-09-21):** dua laporan asal dipanen; K-2 belum dibantah-balik, jangan centang tugas. Pemilik integrator T1-45/T1-30; antrean + syarat bukti wajib di `docs/uji/TINDAK_LANJUT_AUD2_2026-09-21.md`. Sesi ketiga error diabaikan; tanpa audit pengganti/merge/deploy.
  - **Tindak lanjut keputusan Lee (2026-09-21, Batch-5; angka lama dipensiunkan):** T-025(a) diterapkan di `supabase/migrations/0022_beku_setelah_bayar.sql` dan diperkuat pagar akhir satu-pernyataan `0024_beku_satu_pernyataan.sql`; identitas kosong RPC diperketat di `0025_isolasi_identitas_null.sql`, dan deadlock rincian diuji/dipagari `0026_kunci_lingkup_rincian.sql`. Bukti angka **wajib dibaca dari ringkasan runner/CI**, bukan menyalin 64/64 lama: `node alat/uji-sql.mjs`, `alat/uji-mutasi-0022.py`, `alat/uji-mutasi-0024.py`, `alat/uji-mutasi-0025.py`, `alat/uji-konkuren-0022.py`, dan `alat/uji-konkuren-a04.py`. **Gerbang tersisa milik agent T1-45:** audit independen pada commit ber-CI-hijau + verifikasi/sebar Supabase hanya dengan izin Lee; jangan centang tugas dari audit sendiri.
  ...
  - **Progres 2026-09-21 (maraton solo Batch-1 — F F-18 tinggal keputusan Lee):** sisa oracle boolean PIN diverifikasi ulang terhadap definisi HIDUP (`0018_perangkat_terdaftar.sql:336` — auditor mengutip `0015` yang sudah tertimpa): bit kembar-vs-tidak masih bocor lewat `'PIN tersimpan.'` vs penolakan netral, tetapi biayanya berlapis — wajib PIN lama sendiri + wajib perangkat terdaftar + ≤20 tebakan/15 mnt + hanya kandidat kuat yang dijawab + tiap tebakan tercatat permanen beralasan di `percobaan_simpan_pin` (tak bisa dibaca klien) + jawaban tak menyebut milik siapa. Kontrak baru = ubah kontrol keamanan → butuh persetujuan Lee (msg 16); tanpa itu TIDAK ADA penutup kode yang sah. Brief 3 opsi (A terima-sebagai-risiko / B simpan-async "diproses" / C cabut-keunikan-PIN) + rekomendasi agent (A) diajukan ke Lee di laporan batch ini; temuan tetap TERBUKA-menunggu-jawaban, penutup mendarat setelah Lee memilih.
  ...
  - **Progres 2026-09-21 (B F-11 DITUTUP — duplikat K F-03, Batch-2; angka lama dipensiunkan):** lapis PIN-perangkat memakai nama kiriman klien = cacat yang sama dengan K F-03; obat permanen (0018) + uji diperketat + harness mutasi sudah mendarat. Status aktif dibaca dari `docs/uji/AUDIT_RIWAYAT.md` dan keluaran runner terbaru; angka suite 62/62 di baris sejarah ini bukan angka keadaan sekarang.
  ...
  - **Progres 2026-09-21 (F F-12 DIPAGARI-TERBUKA, Batch-2):** probe K-1 DUGAAN selesai — paruh-hitung SUDAH dikunci §6, paruh-cap LUBANG HIDUP (cap diputus di BEFORE tanpa kunci; kunci AFTER terlambat; `hitung_total` hanya clamp; nol konstrain) → diperbaiki `0021_kunci_diskon.sql` (kunci di pemicu BEFORE pertama, pola bayar-0012) + dipagar (`uang_kunci.sql`: 2 sifat `for update` tahan-komentar + asap terima/tolak; `uji-mutasi-0021.py`: 2 mutasi MERAH; suite 63/63; gerbang CI 67). Pelajaran pagar: komentar yang menyebut token sempat membuat mutasi-B hijau-palsu → sifat kini melucuti komentar dulu. Catatan: `create or replace` 0021 sempat menutupi kasus-mutasi F-01 di harness-0015 (kasus 9 hijau-palsu) → sasaran dipindah ke salinan 0021 (aturan baru: mutasi wajib menyasar definisi HIDUP). SISA: uji 2-transaksi-nyata (lingkungan) → temuan tetap TERBUKA.
  - **Progres 2026-09-21 (F F-13 DIVERIFIKASI-TETAP-DIPAGARI, Batch-3 mulai):** pagar nomor-pesanan diverifikasi ulang hijau segar (harness-0015 LOLOS penuh: advisory lock per cabang+tanggal + VOLATILE + kasus-mutasi MERAH-betul; tree identik baterai). Kesimpulan tak berubah: mitigasi hidup, uji 2-transaksi-nyata tetap menunggu lingkungan → TERBUKA-dipagari dipertahankan. Sisa Batch-3 (urut): A F-07 (peta kontrol) → F F-09 (jalur privasi) → I F-17 (keputusan uang) → lapor.
  - **Progres 2026-09-21 (A F-07 DIPETAKAN-PRESISI, Batch-3):** tiap kontrol KEAMANAN dipetakan ke kode: MENDARAT = T1-23 (0011) + T1-24-inti (0018) + T1-27-tabel (0020) + T1-30-sebagian (rahasia+npm+sapuan RLS); SISA = T1-24-sisa + T1-25-penuh + T1-26-unifikasi + T1-27-sisa + T1-28-penuh + T1-29-penuh + T1-30-sisa (definer/search_path) + T2-13 (TOTP). Ketidakadaan diverifikasi (grep 0: sesi, percobaan_masuk, persetujuan, kode, mode_dukungan, matriks, 2 pemeriksa). Temuan samping: KEAMANAN §10 basi ("tabel BELUM ADA") — dilaporkan ke Lee, tak disentuh (§16.1). Temuan tetap TERBUKA-sebagian. Sisa Batch-3: F F-09 → I F-17 → lapor.
  - **Progres 2026-09-21 (F F-09 DITUTUP, Batch-3):** "jalur implementasi" privasi VERIFIKASI ADA & lengkap — kontrak (TECH_SPEC §304 + KEAMANAN §11) + tugas `T8-15` (DoD: kolom persetujuan, fungsi anonimisasi, halaman kebijakan, uji, gerbang tinjau-pemilik) + draf `T-011`; pemilik dikoreksi (`T8-01` = RPC katalog tanpa data pelanggan → `T8-15`). Nol-implementasi hari ini = benar (Fase 8 belum mulai). Kode = lingkup `T8-15` [ ] yang terlacak. Sisa Batch-3: I F-17 → lapor.
  - **Progres 2026-09-21 (I F-17 PARUH-1-HIJAU + T-025, Batch-3):** paruh-1 (lunas/batal tak dihitung-ulang) diverifikasi hijau segar (`urutan_uang.sql` LOLOS + harness-0015 F-01a/b/F-02 LOLOS); sisa jalur-2 (kelebihan bayar) = KEPUTUSAN UANG pemilik → butir baru `T-025` (opsi: larang-ubah-sesudah-bayar / koreksi-tercatat / pengembalian+izin-atasan; rekomendasi agent: larang-dulu sementara) yang MENAHAN `T1-45`. Higiene: baris terbuka T-022/T-023 dipindah ke tabel Butir-terbuka (sebelumnya terselip di tabel Selesai; hitungan parser tetap 8→9). Sisa Batch-3: lapor.
  - **Progres 2026-09-21 (F F-13 DITUTUP, Batch-4 mulai — TEROBOSAN LINGKUNGAN):** PostgreSQL 16.2 nyata hadir di sandbox via pip `pgserver` (apt diblokir egress, sudo ada; pgserver minimal → pgcrypto TIRUAN teks-identik mock terpasang sebagai ekstensi palsu). `alat/uji-konkuren.py` baru: loader skema-penuh (preamble+0021 migrasi+data-uji) + uji F F-13 2-utas (v1=1 v2=2, blokir 1,70 dtk) + kalibrasi wajib-gagal (tanpa kunci → tabrakan+unique-menolak) — LOLOS 6,6 dtk. CI: langkah pip-install + run (gerbang 67→69; pola gerbang dijangkar penuh `^...$`). D F-08 (duplikat) ikut tertutup penuh oleh bukti ini. Sisa Batch-4: F F-12 (uji 2-koneksi) → lapor.
  - **Progres 2026-09-21 (F F-12 DITUTUP, Batch-4):** uji 2-koneksi cap-diskon LOLOS — T1 masuk 30rb, T2 tertahan 1,70 dtk lalu ditolak cap-jumlah (60rb > 54rb), 1 baris; kalibrasi tanpa-kunci: T2 diterima, 2 baris 60rb (111% — skenario Batch-2 TERBUKTI). Satu-sebab dijaga (tumpuk-nyala + cap-resto 50→100 + izin-owner dilonggarkan di DB uji; default-50 dari 0014:305). Pelajaran: server pgserver dipakai-ulang bila direktori tak dibuang (`server_baru` membuang dulu); kunci AFTER-§6 ikut menahan T2 bahkan tanpa kunci-BEFORE (yang membedakan = putusan tolak + jumlah baris, bukan tertahannya). Batch-4 TUNTAS. Sisa: I F-17 (T-025, putusan Lee) → lapor.
  - **Progres 2026-09-21 (PUTUSAN LEE + TUTUP SESI, Batch-4):** Lee "setuju saran terbaik" → 9 butir TERTANGGUH → Selesai (T-025=(a) LARANG; T-023=hapus PIN pelanggan) + KEAMANAN §10 disegarkan (DECISIONS_LOG; §16.1 dipenuhi). I F-17: putusan → implementasi Batch-5 (penegak 0022 + uji; menahan T1-45). Sesi baru (urut): 0022-penegak-T-025(a) → goresan PIN-pelanggan (PRD M10 + TECH_SPEC) → sapu ❓ → T1-30-sisa.

## Fase 2 — Masuk & kerangka aplikasi

- [x] T2-01 — Pemasangan Supabase Auth di klien + penyimpanan sesi aman
  ...
  - **DoD:** sesi tersimpan aman; token disegarkan otomatis; keluar menghapus sesi; tidak ada token di localStorage yang bertahan setelah keluar. · **Bukti 2026-09-22:** `aplikasi/src/lib/auth.ts`, `aplikasi/src/hook/useSesi.ts`, `aplikasi/src/lib/auth.test.ts`, `aplikasi/src/hook/useSesi.test.tsx`.
  - **Kompleksitas:** sedang (3 jam)
...
- [x] T2-02 — Layar masuk pegawai (email + PIN)
  ...
  - **File:** `aplikasi/src/layar/masuk/LayarMasukPegawai.tsx`, `aplikasi/src/layar/masuk/LayarMasukPegawai.test.tsx`
  - **DoD:** masuk dengan email + PIN; PIN salah dibatasi; pesan gagal jelas (bukan pesan teknis); uji alur lulus. · **Bukti 2026-09-22:** `aplikasi/src/layar/masuk/LayarMasukPegawai.tsx`, `aplikasi/src/layar/masuk/LayarMasukPegawai.test.tsx`.
  - **Kompleksitas:** sedang (3 jam)
...
- [x] T2-06 — Kerangka aplikasi: layout, navigasi per peran, tema
  ...
  - **DoD:** menu berbeda untuk 6 peran; halaman awal menyesuaikan peran (kasir → kasir, dapur → dapur); pengalih tema & kerapatan bekerja; akses langsung ke URL terlarang ditolak dengan pesan ramah. · **Bukti 2026-09-22:** `aplikasi/src/App.tsx`, `aplikasi/src/komponen/Rangka.tsx`, `aplikasi/src/komponen/Navigasi.tsx`, `aplikasi/src/komponen/Rangka.test.tsx`, `aplikasi/src/komponen/Navigasi.test.tsx`.
  - **Kompleksitas:** sedang (4 jam)
...
- [x] T2-07 — Pemilih cabang + konteks cabang aktif
  ...
  - **DoD:** Owner/Admin Pusat bisa memilih cabang; Admin Cabang tidak bisa keluar dari cabangnya (server menolak, bukan hanya UI); pilihan tersimpan sesi. · **Bukti 2026-09-22:** `aplikasi/src/hook/useCabang.ts`, `aplikasi/src/komponen/PemilihCabang.tsx`, `aplikasi/src/hook/useCabang.test.tsx`, `aplikasi/src/komponen/PemilihCabang.test.tsx`.
  - **Kompleksitas:** sedang (3 jam)
...
- [x] T2-08 — Halaman "tidak punya akses" + pesan ramah berkode
  ...
  - **DoD:** pesan berisi masalah + tindakan + kode (mis. AK-601); tidak ada istilah teknis; tombol kembali ke halaman yang sesuai. · **Bukti 2026-09-22:** `aplikasi/src/layar/TidakPunyaAkses.tsx`, `aplikasi/src/lib/pesan.ts`, `aplikasi/src/layar/TidakPunyaAkses.test.tsx`, `aplikasi/src/lib/pesan.test.ts`.
  - **Kompleksitas:** kecil (1,5 jam)
...
- [x] T9-02 — Tema & warna merek (10 tema siap pakai)
  ...
  - **File:** `aplikasi/src/layar/pengaturan/Tampilan.tsx`, `aplikasi/src/layar/pengaturan/LayarPengaturan.tsx`, `supabase/migrations/0071_tema_merek.sql`
...
- [x] T9-03 — Pengaturan operasional (pajak, service, pembulatan, cara pesan, struk)  <!-- T-005 sudah ditutup 2026-09-16: nilai awal PB1 10% · service 5% · 1 shift -->
  ...
  - **File:** `aplikasi/src/layar/pengaturan/Operasional.tsx`, `aplikasi/src/layar/pengaturan/LayarPengaturan.tsx`, `supabase/migrations/0072_pengaturan_operasional.sql`
...
- [x] T9-04 — Meja & area + QR per meja
  ...
  - **Ref:** PRD M2 & M4
  - **File:** `aplikasi/src/layar/pengaturan/Meja.tsx`, `aplikasi/src/layar/pengaturan/LayarPengaturan.tsx`, `supabase/migrations/0073_pengaturan_meja.sql`
...
- [x] T9-05 — Pengelolaan menu lengkap (kategori, varian, tambahan, foto, urutan)
  ...
  - **Ref:** PRD M2 (menu)
  - **File:** `aplikasi/src/layar/pengaturan/Menu.tsx`, `supabase/migrations/0074_pengaturan_menu.sql`
...
- [x] T9-08 — Kelola pegawai: peran, izin, PIN ⚠️
  ...
  - **File:** `aplikasi/src/layar/pengaturan/KelolaPegawai.tsx`, `supabase/migrations/0077_kelola_pegawai_izin.sql`
...
- [x] T9-09 — Kelola cabang (tambah, printer, nonaktifkan) ⚠️
  ...
  - **File:** `aplikasi/src/layar/pengaturan/Cabang.tsx`, `supabase/migrations/0078_kelola_cabang.sql` (menggantikan nomor rencana `0063_kelola_cabang.sql` yang sudah terisi migrasi anti email palsu)
