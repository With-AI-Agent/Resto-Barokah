...
--
-- Batas jujur:
--   * Kalau staf memegang banyak cabang, hasilnya = cabang pertama menurut
--     `cabang_ids_saya()` (urutan implementasi). Itu deterministik untuk
--     satu sesi, dan kasir boleh mengirim `p_cabang_id` eksplisit bila
--     ingin presisi.
--   * Kalau staf tidak punya akses ke cabang manapun → `null` (sama
--     perilaku dengan `cabang_saya()`). `kas_pergerakan` selanjutnya
--     akan menolak dengan "tidak ditemukan shift terbuka" — pagar tetap
--     bekerja.
-- ============================================================================
