...
select uji.harap_gagal(
  ...
  'klien TIDAK boleh memindahkan status pesanan menjadi lunas'
);
...
select uji.harap_gagal(
  ...
  'klien tidak boleh melompati urutan status (draf → dimasak)'
);

-- 3. Jalur resmi: kirim ke dapur (draf → dikirim) → DITERIMA, tanda waktunya terisi.
select uji.harap_gagal(
  $$update public.pesanan set status = 'dikirim' where kunci_idempoten = 'status-pesanan-uji'$$,
  'mengirim pesanan WAJIB menyertakan waktu kirim ke dapur'
);
...
select uji.harap_gagal(
  $$update public.pesanan set dikirim_ke_dapur_pada = null where kunci_idempoten = 'status-pesanan-uji'$$,
  'tanda kirim ke dapur tidak boleh dihapus klien'
);
reset role;
...
select uji.harap_gagal(
  $$update public.pesanan set status = 'dimasak' where kunci_idempoten = 'status-pesanan-uji'$$,
  'pelayan tidak boleh memajukan status dapur'
);
reset role;
select uji.klaim(null);
...
set local role authenticated;
select uji.harap_gagal(
  $$update public.pesanan set status = 'draf' where kunci_idempoten = 'status-pesanan-uji'$$,
